const IndexPage = () => {
  ...
  const prgManagerMain = document.querySelector(
    "#prgManagerMain > div.cloumnSubjects.column"
  );
  if (prgManagerMain) {
  ...
  document
    .querySelectorAll("#cloumnSubjectInfo .blockMode")
    .forEach((i) => (i.style.height = "80vh"));
  ...
  document.querySelectorAll("#prgSubjectList li").forEach((i) => {
    ...
    // i.style.order = String(
    //   1 - Math.floor(Number(percent.style.width.replace("%", "")))
    // );
  });

  document
    .querySelectorAll("#cloumnSubjectInfo > div .overlay")
    .forEach((i) => i.remove());
  document
    .querySelectorAll("#cloumnSubjectInfo > div .image")
    .forEach((i) => (i.style.height = "120px"));
  document
    .querySelectorAll("#cloumnSubjectInfo > div .pictureFrameGroup")
    .forEach((i) => (i.style.height = "120px"));
  document
    .querySelectorAll("#cloumnSubjectInfo > div img")
    .forEach((i) =>
      i.setAttribute("src", i.getAttribute("src").replace(/\/c\//, "/l/"))
    );
  ...
  document
    .querySelectorAll(".headerInner")
    .forEach((i) => (i.style.marginLeft = "130px"));
};
